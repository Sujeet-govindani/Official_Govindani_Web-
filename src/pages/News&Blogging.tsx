...
function MobileCard({
  ...
  const triggerScrollHint = useCallback(() => {
    ...
    const goDown = () => {
      ...
      else { el.scrollTop = target; setTimeout(() => { const goUp = () => { el.scrollTop = el.scrollTop * 0.84; if (el.scrollTop > 0.5) scrollAnimRef.current = requestAnimationFrame(goUp); else el.scrollTop = 0; }; scrollAnimRef.current = requestAnimationFrame(goUp); }, 520); }
    };
    cancelAnimationFrame(scrollAnimRef.current);
    scrollAnimRef.current = requestAnimationFrame(goDown);
  }, [scrolled]);

  useEffect(() => () => { cancelAnimationFrame(scrollAnimRef.current); if (tapTimerRef.current) clearTimeout(tapTimerRef.current); }, []);

  const handleTap = useCallback((e: React.TouchEvent | React.MouseEvent) => {
    if (e.type === "touchend") e.preventDefault();
    const now = Date.now(); const gap = now - lastTapRef.current; lastTapRef.current = now;
    if (gap < 320 && gap > 0) { if (tapTimerRef.current) clearTimeout(tapTimerRef.current); cancelAnimationFrame(scrollAnimRef.current); window.open(project.url, "_blank", "noopener,noreferrer"); }
    else { if (tapTimerRef.current) clearTimeout(tapTimerRef.current); tapTimerRef.current = setTimeout(() => triggerScrollHint(), 200); }
  }, [project.url, triggerScrollHint]);

  return (
    <div ref={cardRef} className="mob-card-anim"
      style={{ transitionDelay: `${(index % 3) * 0.08}s`, opacity: visible ? 1 : 0, transform: visible ? "translateY(0) scale(1)" : "translateY(48px) scale(0.95)" }}>
      <div className="mob-img-outer">
        <div ref={scrollRef} className="mob-scroll-container" onTouchEnd={handleTap} onClick={handleTap}>
          <div className="mob-img-wrap"><img src={project.image} alt={project.title} className="mob-img" draggable={false} loading="lazy" decoding="async" /></div>
        </div>
        ...
        {hintVisible && (
          <div className={`mob-hint ${hintPulse ? "mob-hint-pulse" : ""}`}>
            <span className="mob-hint-scroll"><svg width="13" height="20" viewBox="0 0 13 20" fill="none"><rect x="1" y="1" width="11" height="16" rx="5.5" stroke="currentColor" strokeWidth="1.4"/><line x1="6.5" y1="4" x2="6.5" y2="8" stroke="currentColor" strokeWidth="1.4" strokeLinecap="round"/></svg>Scroll</span>
            <span className="mob-hint-divider">·</span>
            <span className="mob-hint-dbl"><svg className="tap-ico" width="15" height="15" viewBox="0 0 24 24" fill="none" aria-hidden="true">
              ...
              </svg>Double tap to visit</span>
          </div>
        )}
        <div className="mob-info"><h3 className="mob-title">{project.title}</h3><span className="mob-arrow">↗</span></div>
...
// ─── Desktop Card ─────────────────────────────────────────────────────────────
function DesktopCard({ project, index, isVisible }: { project: { title: string; category: string; url: string; image: string }; index: number; isVisible: boolean; }) {
  ...
  const animateCursor = useCallback(() => { cursorPos.current.x += (mousePos.current.x - cursorPos.current.x) * 0.12; cursorPos.current.y += (mousePos.current.y - cursorPos.current.y) * 0.12; setCursor(prev => ({ ...prev, x: cursorPos.current.x, y: cursorPos.current.y })); cursorAnimRef.current = requestAnimationFrame(animateCursor); }, []);
  const animateScrollReset = useCallback(() => { if (!scrollRef.current) return; const cur = scrollRef.current.scrollTop; if (cur > 0.5) { scrollRef.current.scrollTop = cur * 0.88; scrollAnimRef.current = requestAnimationFrame(animateScrollReset); } else { scrollRef.current.scrollTop = 0; } }, []);
  const handleMouseEnter = useCallback(() => { cancelAnimationFrame(scrollAnimRef.current); setCursor(prev => ({ ...prev, opacity: 1 })); cursorAnimRef.current = requestAnimationFrame(animateCursor); }, [animateCursor]);
  const handleMouseLeave = useCallback(() => { cancelAnimationFrame(cursorAnimRef.current); setCursor(prev => ({ ...prev, opacity: 0 })); scrollAnimRef.current = requestAnimationFrame(animateScrollReset); }, [animateScrollReset]);
  const handleMouseMove  = useCallback((e: React.MouseEvent) => { if (!containerRef.current) return; const rect = containerRef.current.getBoundingClientRect(); mousePos.current = { x: e.clientX - rect.left, y: e.clientY - rect.top }; }, []);
  useEffect(() => () => { cancelAnimationFrame(cursorAnimRef.current); cancelAnimationFrame(scrollAnimRef.current); }, []);

  return (
    <div className={`nb-card-anim ${isVisible ? "visible" : ""}`} style={{ transitionDelay: `${index * 0.09}s` }}>
      <div ref={containerRef} className="nb-card-outer" onMouseEnter={handleMouseEnter} onMouseLeave={handleMouseLeave} onMouseMove={handleMouseMove} onClick={() => window.open(project.url, "_blank", "noopener,noreferrer")}>
        <div ref={scrollRef} className="nb-card-inner-scroll">
          <div className="nb-card-img-wrap"><img src={project.image} alt={project.title} className="nb-card-img" draggable={false} loading="lazy" decoding="async" /></div>
          <div className="nb-card-grad-top" /><div className="nb-card-grad-bot" />
          ...
          <div className="nb-card-info"><h3 className="nb-card-title">{project.title}</h3><span className="nb-card-arrow">↗</span></div>
        </div>
        <div className="nb-custom-cursor" style={{ left: cursor.x, top: cursor.y, opacity: cursor.opacity }}>
          <div className="nb-cursor-circle"><svg width="18" height="26" viewBox="0 0 18 26" fill="none"><rect x="1" y="1" width="16" height="22" rx="8" stroke="white" strokeWidth="1.5"/><line x1="9" y1="5" x2="9" y2="10" stroke="white" strokeWidth="1.5" strokeLinecap="round"/></svg></div>
        </div>
      </div>
    </div>
  );
...
function AnimatedCard({ project, index, isVisible }: { project: { title: string; category: string; url: string; image: string }; index: number; isVisible: boolean; }) {
  ...
  return touch ? <MobileCard project={project} index={index} /> : <DesktopCard project={project} index={index} isVisible={isVisible} />;
}

// ─── Search Bar ───────────────────────────────────────────────────────────────
function NewsSearchBar({ onSearch }: { onSearch: (query: string) => void }) {
  const [query,   setQuery]   = useState("");
  const [focused, setFocused] = useState(false);
  const [sass,    setSass]    = useState("Search for news, blogs, topics...");
  const inputRef = useRef<HTMLInputElement>(null);

  const sassMessages = [
    "Searching across the universe...",
    "Finding your perfect read...",
    "Scanning every article ever...",
    "Boop beep bop boop.",
  ];

  const handleSearch = () => {
    if (!query.trim()) { setSass("You can't search nothing, you silly goose."); return; }
    setSass(sassMessages[Math.floor(Math.random() * sassMessages.length)]);
    onSearch(query.trim());
  };
  ...
  const handleChange = (val: string) => {
    setQuery(val);
    if (!val.trim()) setSass("Type something to search...");
    else setSass("Hit enter to search · Are you sure? · Looking good!");
  };

  const handleReset = () => { setQuery(""); setSass("Search for news, blogs, topics..."); onSearch(""); };

  return (
    <div className={`nb-searchbar-aligner ${focused ? "focused" : ""} ${query ? "querying" : ""}`}>
      <div className="nb-searchbar-wrapper">
        <div className="nb-searchbar">
          <svg className="nb-search-icon" width="16" height="16" viewBox="0 0 16 16" fill="none">
            <circle cx="6.5" cy="6.5" r="5" stroke="rgba(255,255,255,0.7)" strokeWidth="1.5"/>
            <line x1="10.5" y1="10.5" x2="14" y2="14" stroke="rgba(255,255,255,0.7)" strokeWidth="1.5" strokeLinecap="round"/>
          </svg>
          <input
            ref={inputRef}
            ...
            onFocus={() => { setFocused(true); setSass("Good day. Start typing to search."); }}
            onBlur={() => setFocused(false)}
            onKeyDown={handleKeyDown}
          />
          {query && (
            <button className="nb-search-reset" onClick={handleReset} type="button">
        ...
        <div className="nb-search-sass">
          <svg width="14" height="14" viewBox="0 0 14 14" fill="none" style={{ flexShrink: 0 }}>
            <circle cx="7" cy="7" r="6" stroke="rgba(212,175,55,.6)" strokeWidth="1.2"/>
            <path d="M7 4v3.5l2 2" stroke="rgba(212,175,55,.6)" strokeWidth="1.2" strokeLinecap="round"/>
          </svg>
          <span>{sass}</span>
        </div>
      </div>
...
// ─── Main News & Blogging Page ────────────────────────────────────────────────
const NewsAndBloggingPage = () => {
  ...
  const allProjects = [
    { title: "THE DAILY PULSE",     category: "news",    url: "https://dailypulse.com",      image: "https://pub-8d8c06eb82144fca803dab6ccecd7b41.r2.dev/Images/News-1.png"    },
    { title: "INSIGHT TRIBUNE",     category: "news",    url: "https://insighttribune.com",   image: "https://pub-8d8c06eb82144fca803dab6ccecd7b41.r2.dev/Images/News-2.png"    },
    { title: "GLOBE CHRONICLE",     category: "news",    url: "https://globechronicle.com",   image: "https://pub-8d8c06eb82144fca803dab6ccecd7b41.r2.dev/Images/News-3.png"    },
    { title: "VIVID NARRATIVES",    category: "blog",    url: "https://vividnarratives.com",  image: "https://pub-8d8c06eb82144fca803dab6ccecd7b41.r2.dev/Images/Blog-1.png"    },
    { title: "THE THOUGHT VAULT",   category: "blog",    url: "https://thoughtvault.com",     image: "https://pub-8d8c06eb82144fca803dab6ccecd7b41.r2.dev/Images/Blog-2.png"    },
    { title: "PIXEL STORIES",       category: "blog",    url: "https://pixelstories.com",     image: "https://pub-8d8c06eb82144fca803dab6ccecd7b41.r2.dev/Images/Blog-3.png"    },
    { title: "METRO DIGEST",        category: "news",    url: "https://metrodigest.com",      image: "https://pub-8d8c06eb82144fca803dab6ccecd7b41.r2.dev/Images/News-4.png"    },
    { title: "SPARK JOURNAL",       category: "blog",    url: "https://sparkjournal.com",     image: "https://pub-8d8c06eb82144fca803dab6ccecd7b41.r2.dev/Images/Blog-4.png"    },
    { title: "ECHO PRESS",          category: "news",    url: "https://echopress.com",        image: "https://pub-8d8c06eb82144fca803dab6ccecd7b41.r2.dev/Images/News-5.png"    },
  ];
  ...
  useEffect(() => {
    ...
    const obs = items.map(({ ref, setter }) => {
      const o = new IntersectionObserver(([e]) => { if (e.isIntersecting) setter(true); }, { threshold: 0.08 });
  ...
  useEffect(() => { const t = setTimeout(() => setSec1Visible(true), 100); return () => clearTimeout(t); }, []);
    ...
    <>
      <style>{`
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        @keyframes nbSearchPulse {
          0%,100%{ box-shadow:0 0 0 0 rgba(212,175,55,0); }
          50%    { box-shadow:0 0 0 6px rgba(212,175,55,.08); }
        }
        @keyframes sassFade { from{opacity:0;transform:translateX(-10px)} to{opacity:1;transform:translateX(0)} }
        ...
        /* ════════════════════════════
           SEARCH BAR
        ════════════════════════════ */
        .nb-searchbar-aligner {
          display:flex; justify-content:center;
          width:100%; margin-bottom:40px;
          position:relative; z-index:10;
        }
        .nb-searchbar-wrapper {
        ...
        .nb-searchbar-aligner.focused .nb-searchbar,
        .nb-searchbar-aligner.querying .nb-searchbar {
          border-color:rgba(212,175,55,.55);
          box-shadow:0 0 0 3px rgba(212,175,55,.08);
        }
        .nb-search-icon { flex-shrink:0; }
        .nb-search-input {
        ...
        .nb-search-reset:hover { background:rgba(255,255,255,.18); }
        .nb-search-sass {
          display:flex; align-items:center; gap:7px;
          padding:0 6px;
          animation:sassFade .3s ease;
        }
        .nb-search-sass span {
          font-family:'Inter',sans-serif; font-size:.78rem;
          color:rgba(212,175,55,.7); line-height:1.4;
        }
        ...
        .nb-hero-section::before {
          content:''; position:absolute; inset:0;
          background:radial-gradient(ellipse at 50% 50%,rgba(212,175,55,.04) 0%,transparent 60%);
          pointer-events:none;
        }
        /* ── Mobile: reset padding, spacer div handles navbar clearance ── */
        @media(max-width:640px){
          .nb-hero-section { padding:20px 14px 60px; justify-content:flex-start; min-height:unset; }
        }
        ...
        .nb-particle {
          ...
          animation:nbFloatY var(--dur,5s) ease-in-out var(--delay,0s) infinite;
        }

        /* Glassy card */
        .nb-hero-card {
          position:relative; z-index:2;
          width:96%; max-width:1260px;
          padding:52px 68px; border-radius:22px; overflow:hidden;
          background:rgba(255,255,255,.04);
          backdrop-filter:blur(10px) saturate(1.5);
          -webkit-backdrop-filter:blur(10px) saturate(1.5);
          border:1px solid rgba(212,175,55,.16);
          animation:nbBorderGlow 4.5s ease-in-out infinite;
        ...
        .nb-hero-card-bg {
          ...
          background-image:url('https://pub-8d8c06eb82144fca803dab6ccecd7b41.r2.dev/Images/news1.jfif');
          background-size:cover; background-position:center;
          opacity:0.40; z-index:0; pointer-events:none;
        }
        .nb-hero-card-overlay {
          position:absolute; inset:0;
        ...
        .nb-hero-card-content {
          position:relative; z-index:1;
          display:grid; grid-template-columns:1fr 1fr;
          gap:64px; align-items:center;
        }
        @media(max-width:900px){
          .nb-hero-card-content{grid-template-columns:1fr;gap:36px;}
          .nb-hero-card{padding:40px 28px;}
        }
        ...
        .nb-hero-title {
          ...
          font-size:clamp(1.9rem,3.5vw,3rem); font-weight:700; line-height:1.18; color:#fff; margin-bottom:12px;
        }
        .nb-hero-title span {
          background:linear-gradient(90deg,#d4af37,#f4e5b8,#d4af37); background-size:200% auto;
          -webkit-background-clip:text; background-clip:text; -webkit-text-fill-color:transparent;
          animation:nbShimmer 4s linear infinite;
        }
        .nb-hero-subtitle {
          font-family:'Libre Baskerville',serif; font-style:italic;
          font-size:clamp(.95rem,1.6vw,1.1rem); color:rgba(212,175,55,.75); margin-bottom:20px; line-height:1.5;
        }
        ...
        .nb-info-section::before { content:'';position:absolute;inset:0;background:radial-gradient(ellipse at 80% 50%,rgba(212,175,55,.04) 0%,transparent 60%);pointer-events:none; }
        ...
        .nb-content-right.visible { opacity:1;transform:translateX(0); }
        ...
        .nb-do-you-know { font-family:'Libre Baskerville',serif;font-style:italic;font-size:1rem;color:rgba(212,175,55,.65);margin-bottom:6px; }
        ...
        /* ════════════════════════════
           SECTION 3 9 CARDS
        ════════════════════════════ */
        .nb-projects-section { position:relative;background:linear-gradient(to bottom,#000,#08070d,#000);padding:100px 28px 80px;overflow:hidden; }
        ...
        .nb-projects-sub { font-family:'Libre Baskerville',serif;font-style:italic;font-size:1.05rem;color:rgba(212,175,55,.65);margin-bottom:0; }

        .nb-grid { display:grid;grid-template-columns:repeat(3,1fr);gap:28px;max-width:1200px;margin:48px auto 0; }
        @media(max-width:1024px){ .nb-grid{grid-template-columns:repeat(2,1fr);} }
        @media(max-width:640px)  { .nb-grid{grid-template-columns:1fr;} }

        .nb-no-results { grid-column:1/-1;text-align:center;padding:48px 0; }
        .nb-no-results p { font-family:'Inter',sans-serif;font-size:.95rem;color:rgba(255,255,255,.4); }

        /* Cards */
        .nb-card-anim { opacity:0;transform:translateY(24px) scale(.97);transition:opacity .5s ease,transform .5s ease; }
        .nb-card-anim.visible { opacity:1;transform:translateY(0) scale(1); }
        .nb-card-outer { position:relative;height:380px;border-radius:16px;overflow:hidden;cursor:none;background:#0a0a0a;box-shadow:0 8px 32px rgba(0,0,0,.6);transition:box-shadow .3s ease,transform .3s ease; }
        ...
        .nb-card-title { font-family:'Libre Baskerville',serif;font-size:14px;font-weight:700;color:#fff;line-height:1.3;text-shadow:0 1px 10px rgba(0,0,0,1);transition:color .3s ease; }
        .nb-card-arrow { font-size:20px;color:#d4af37;text-shadow:0 0 12px rgba(212,175,55,.8);flex-shrink:0; }
        .nb-card-outer:hover { box-shadow:0 16px 48px rgba(212,175,55,.2);transform:translateY(-4px); }
        ...
        /* Mobile card styles */
        .mob-card-anim { transition:opacity .58s ease,transform .58s cubic-bezier(0.22,1,0.36,1);border-radius:18px;overflow:hidden;-webkit-tap-highlight-color:transparent;touch-action:pan-y; }
        .mob-img-outer { position:relative;height:320px;border-radius:18px;overflow:hidden;background:#111;border:2px solid rgba(212,175,55,.4);box-shadow:0 10px 40px rgba(0,0,0,.7),0 0 0 1px rgba(212,175,55,.1); }
        ...
        .nb-closing-bg { position:absolute;inset:0;background-image:url('https://pub-8d8c06eb82144fca803dab6ccecd7b41.r2.dev/Images/news2.jfif');background-size:cover;background-position:center;opacity:0.45;z-index:0; }
        ...
        .nb-torn-card {
          position:relative; width:min(500px,90vw); padding:56px 52px 52px 44px;
          ...
          border-left:1px solid rgba(212,175,55,.18); border-top:1px solid rgba(212,175,55,.14); border-bottom:1px solid rgba(212,175,55,.14);
          animation:nbTornFlicker 4s ease-in-out infinite;
...
`}</style>
      ...
      <div className="nb-page">
        ...
        <section className="nb-hero-section">
          ...
          {/*
            ── NAVBAR SPACER ─────────────────────────────────────────────────
            Inline style = highest CSS specificity, no external sheet can
            override it. Pushes the search bar below the fixed site header.
            Tune `height` to match your navbar height + breathing room.
          ──────────────────────────────────────────────────────────────────── */}
            ...
            {[
              ...
              <div key={i} className="nb-particle"
                style={{ left:p.left, top:p.top, width:p.w, height:p.w,
                  "--dur":p.dur, "--delay":p.delay } as React.CSSProperties} />
            ))}
          </div>

          {/* ── Search Bar ── */}
          <NewsSearchBar onSearch={(q) => setSearchQuery(q)} />
            ...
            <div className="nb-hero-card-content">
              <div>
                <div className="nb-hero-eyebrow">
                  ...
                  <span>News & Blogging Solutions</span>
                </div>
                <h1 className="nb-hero-title">
                  Our <span>News & Blogging</span> Websites
                </h1>
                <p className="nb-hero-subtitle">Stay Updated with Engaging and Informative Content</p>
                <div className="nb-hero-divider-line"><div className="nb-hero-divider-diamond" /></div>
                <div className="nb-hero-features">
                  {[
                    { icon:"📰", label:"Breaking News",      sub:"Real-time updates delivered fast"        },
                    { icon:"✍️",  label:"Lifestyle Blogs",    sub:"Personal & brand storytelling"           },
                    { icon:"🌍",  label:"Global Audience",    sub:"Multi-language, multi-platform reach"    },
                    { icon:"🔔",  label:"Smart Alerts",       sub:"Push notifications & subscriptions"     },
                  ].map(f => (
                    <div key={f.label} className="nb-hero-feature">
                      <div className="nb-feature-icon">{f.icon}</div>
              ...
              <div>
                <p className="nb-hero-para">
                  We create dynamic and user-friendly news and blogging websites designed to cater
                  to a global audience. Whether it's breaking news, lifestyle blogs, or expert
                  opinions, our platforms deliver content that informs, inspires, and connects
                  readers worldwide.
                </p>
              </div>
            </div>
        ...
        {/* ════ SECTION 2 VIDEO LEFT + CONTENT RIGHT ════ */}
        <section className="nb-info-section">
          <div className="nb-info-inner">
            <div ref={videoRef} className={`nb-video-wrap ${videoVisible ? "visible" : ""}`}>
              <div className="nb-video-badge">Watch Overview</div>
              <div className="nb-video-corner-top" />
              <iframe
                src="https://www.youtube.com/embed/dQw4w9WgXcQ"
                title="News & Blogging Overview"
                allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture"
                allowFullScreen
              />
            </div>
            <div ref={sec2Ref} className={`nb-content-right ${sec2Visible ? "visible" : ""}`}>
              <div className="nb-tag-pill">
                <div style={{ width:6, height:6, borderRadius:"50%", background:"#d4af37" }} />
                <span>Did You Know?</span>
              </div>
              <p className="nb-do-you-know">Do You know</p>
              <h2 className="nb-sec2-title">
                What is a <em>News & Blogging Website?</em>
              </h2>
              <div className="nb-sec2-divider" />
              <p className="nb-sec2-para">
                A News & Blogging website is a digital platform that hosts timely updates, articles,
                opinions, and insights on a variety of topics. These websites serve as hubs of
                information and personal expression, catering to audiences seeking fresh perspectives,
                expert opinions, and engaging stories. They can focus on specific niches like
                technology, lifestyle, or entertainment, or cover a wide range of subjects.
              </p>
              <p className="nb-sec2-para">
                For news, such websites act as real-time sources of updates on current events,
                ensuring readers stay informed. On the other hand, blogging provides a platform for
                individuals or businesses to share experiences, ideas, and expertise, connecting with
                audiences on a deeper, more personal level.
              </p>
              <p className="nb-sec2-cta">Have a look at our News And Blogging projects</p>
            </div>
          </div>
        </section>

        {/* ════ SECTION 3 9 CARDS ════ */}
        <section className="nb-projects-section">
          <div ref={sec3Ref} className={`nb-projects-header ${sec3Visible ? "visible" : ""}`}>
            ...
            <h2 className="nb-projects-title">
              Have a look at our <span>News And Blogging</span> Projects
            </h2>
            <p className="nb-projects-sub">News & Blogging Websites</p>
          </div>

          <div className="nb-grid">
            {displayProjects.length === 0 ? (
              <div className="nb-no-results">
                <p>No projects match "<strong style={{ color:"#d4af37" }}>{searchQuery}</strong>" try a different search.</p>
              </div>
            ) : (
              displayProjects.map((project, index) => (
                <AnimatedCard key={project.title} project={project} index={index} isVisible={sec3Visible} />
              ))
            )}
          </div>
          ...
          <div className={`nb-more-note ${sec3Visible ? "visible" : ""}`}>
            <p>
              We have <strong>9+ projects</strong> in News & Blogging and growing every month.
              Can't find what you're looking for? <strong>Connect with us</strong> to explore our
              full portfolio and discover the perfect solution for your platform.
            </p>
          </div>
        </section>
...
export default NewsAndBloggingPage;